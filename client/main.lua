...
local function RegisterEntranceTarget()
    entranceZone = BoxZone:Create(vector3(Config.OutsideLocation.x, Config.OutsideLocation.y, Config.OutsideLocation.z),
        1, 4, {
...
RegisterNetEvent('qb-recyclejob:client:target:pickupPackage', function()
    QBCore.Functions.Progressbar('pickup_reycle_package', 'Pick Up The Package ..', math.random(4000, 6000), false,
        true, {
            ...
            if onDuty then
                if not pickupZone and not carryPackage then
                    ...
                    elseif packageCoords and not carryPackage then
                        sleep = 0
                        DrawMarker(2, packageCoords.x, packageCoords.y, packageCoords.z + 3, 0, 0, 0, 180.0, 0, 0, 0.5,
                            0.5, 0.5, 255, 255, 0, 100, false, false, 2, true, nil, nil, false)
                    end
